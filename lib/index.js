...
module.exports = {
    ...
    req: function(req, json) {
        ...
        _console(color, address, method, msg);
    },
...
var _console = function(color, step1, step2, msg) {
    log = _logDate();
    log += _logStep(color, step1);
    log += _logStep(color, step2);
    log += _logMessage(msg);
    console.log(log);
};
...
var _logMessageReq = function(req, json) {
    ...
    return _logTag(url) + ' ' + _logTag(data);
};
...
var _replacePassword = function(data) {
    console.log('restberry-logger: why enter here so many times?')
    for (key in data) {
        if (key === PASSWORD_KEY) {
